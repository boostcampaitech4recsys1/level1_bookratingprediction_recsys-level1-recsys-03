...
def process_dl_data(users, books, ratings1, ratings2): # label-encoding 
...
def dl_data_load(args):
    ...
    l = list(books.groupby('category')['category'].count().sort_values(ascending=False).head(60).index) # 70
    books['category'] = books['category'].apply(get_core)
    l = list(books.groupby('language')['language'].count().sort_values(ascending=False).head(9).index) # 9
    books['language'] = books['language'].apply(get_core)
    l = list(books.groupby('publisher')['publisher'].count().sort_values(ascending=False).head(176).index) # 200
    books['publisher'] = books['publisher'].apply(get_core)
    l = list(books.groupby('book_title')['book_title'].count().sort_values(ascending=False).head(91).index) # 100
    books['book_title'] = books['book_title'].apply(get_core)
    l = list(users.groupby('location_country')['location_country'].count().sort_values(ascending=False).head(31).index) # 32
    users['location_country'] = users['location_country'].apply(get_core)
    l = list(users.groupby('location_state')['location_state'].count().sort_values(ascending=False).head(138).index) # 100
    users['location_state'] = users['location_state'].apply(get_core)
    l = list(users.groupby('location_city')['location_city'].count().sort_values(ascending=False).head(500).index) # 600
    users['location_city'] = users['location_city'].apply(get_core)
    l = list(books.groupby('book_author')['book_author'].count().sort_values(ascending=False).head(350).index) # 400
    ...
    idx, context_train, context_test = process_dl_data(users, books, train, test) # users, books, ratings1, ratings2

    field_dims = np.array([len(user2idx), len(isbn2idx),
                            1, len(idx['loc_city2idx']), len(idx['loc_state2idx']), len(idx['loc_country2idx']),
                            len(idx['category2idx']),len(idx['publisher2idx']), len(idx['language2idx']), len(idx['author2idx']), len(idx['title2idx']), len(idx['year_of_publication'])], dtype=np.uint32)
