...
class DeepFM:
    ...
    def __init__(self, args, data):
        ...
        self.embed_dim = args.DeepFM_EMBED_DIM
        self.epochs = args.EPOCHS
        self.learning_rate = args.LR
        self.weight_decay = args.WEIGHT_DECAY
        self.log_interval = 100
        ...
        self.mlp_dims = args.DeepFM_MLP_DIMS
        self.dropout = args.DeepFM_DROPOUT

        self.model = _DeepFM(self.field_dims, self.embed_dim, mlp_dims=self.mlp_dims, dropout=self.dropout).to(self.device)
        self.optimizer = torch.optim.AdamW(params=self.model.parameters(), lr=self.learning_rate, amsgrad=True, weight_decay=self.weight_decay)
