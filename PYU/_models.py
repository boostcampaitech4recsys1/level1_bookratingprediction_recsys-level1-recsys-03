...
class _KNNCollaborativeFiltering(nn.Module):
    def __init__(self):
        super().__init__()
...
class _DeepFM(torch.nn.Module):
    def __init__(self, field_dim_list, embed_dim, mlp_dims, dropout):
        super().__init__()
        self.linear = FeaturesLinear(field_dim_list)
        self.fm = FactorizationMachine(reduce_sum=True)
        self.embedding = FeaturesEmbedding(field_dim_list, embed_dim)
